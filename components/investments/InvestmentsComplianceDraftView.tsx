...
const LIVE_PAGE_GAPS = [
  {
    area: "Hero and CTAs",
    live: "Sales framing (“private-market income”), “Compare investment profiles”, and an Everest-first prompt.",
    draft:
      "Factual service framing, education-first CTAs, and no product-first or Everest promotional prompt.",
  },
  {
    area: "Suitability language",
    live: "Uses phrases such as “Where suitable”, “Suited when”, and competitor contrast (“generic IFAs”).",
    draft:
      "Removed suitability and comparative puffery. Product selection is deferred to a needs analysis.",
  },
  {
    area: "Named product shelf",
    live: "Promotes Everest yield rows (12.8% / 14.2% / 14.5%) and Amethyst as browseable options.",
    draft:
      "No named-product yield shelf. Explains wrapper types and unlisted-security risks only. Product pages remain under containment review.",
  },
  {
  ...
  },
  {
    ...
    live: "FAIS calculator note present; page-level general advice disclaimer is not prominent.",
    draft:
      "Prominent Section 1(3)(a) factual-information disclaimer near the top, repeated near tools and contact.",
  },
  {
    area: "Risk disclosure",
    live: "Liquidity notes appear beside marketed yields.",
    draft:
      "Illiquidity, valuation, notice, early-exit, and “not guaranteed” warnings appear before any product discussion.",
  },
] as const;
...
/**
 * Compliance draft of the Investments hub for Frank / Albert review.
 * Does not replace /investments. Keep noindex until approved.
 */
export function InvestmentsComplianceDraftView({ faqs }: Props) {
  const faqItems = ensureSixFaqs(faqs);

  return (
    <div style={{ backgroundColor: CANVAS }} className="overflow-x-clip text-shark">
      <div className="border-b border-amber-200 bg-amber-50">
        <div className={`${HOME4_WRAP} py-4`}>
      ...
      <MarketingHubHero
        kicker={<HubHeroKicker shortLabel="Investments" longLabel="Investments · Compliance draft" />}
        ...
        actions={
          <HubHeroActions
            ...
            primaryHref="/contact?source=investments_compliance_draft"
          />
        }
        visual={
      ...
      >
        <div className={HOME4_WRAP}>
          <p
            ...
            style={{ color: TEAL }}
          >
            Reviewer notes · Live page vs this draft
          </p>
          <h2
            id="gap-heading"
            className="mt-3 font-serif font-semibold tracking-tight"
            style={{ fontSize: "clamp(1.5rem, 1.25rem + 1vw, 2.125rem)", color: INK }}
          >
            What this draft changes for compliance review
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-relaxed" style={{ color: BODY }}>
            These notes are for Frank and Albert. They are not public marketing copy. Sources:
            FAIS/GCOC digital compliance research docs and the 2026-07-22 containment instruction.
          </p>
          <div className="mt-8 overflow-hidden border bg-white" style={{ borderColor: HAIRLINE }}>
            <div
            ...
            >
              ...
              <div className="col-span-5">Current /investments</div>
              <div className="col-span-5">This draft</div>
            </div>
            {LIVE_PAGE_GAPS.map((row) => (
              <div
                key={row.area}
                className="grid grid-cols-1 gap-3 border-b px-4 py-5 last:border-b-0 md:grid-cols-12 md:gap-6"
                style={{ borderColor: HAIRLINE }}
              >
                <p className="text-sm font-semibold text-shark md:col-span-2">{row.area}</p>
                <p className="text-sm leading-relaxed text-stone-600 md:col-span-5">
                  <span className="md:hidden font-semibold text-stone-500">Live: </span>
                  {row.live}
                </p>
                <p className="text-sm leading-relaxed text-stone-600 md:col-span-5">
                  <span className="md:hidden font-semibold text-stone-500">Draft: </span>
                  {row.draft}
                </p>
              </div>
            ))}
        ...
        <div className={HOME4_WRAP}>
          ...
          >
            ...
            <div className="mt-8 shrink-0 md:mt-0">
              <Link
                href="/contact?source=investments_compliance_draft_terminal"
                prefetch={false}
